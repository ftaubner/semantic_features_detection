...
class InteriorDataset(utils.Dataset):
    ...
    def load_Interior(self, dataset_dir, subset, class_ids=None, return_coco=False):
    ...
    def load_mask(self, image_id):
        ...
        for annotation in annotations:
            class_id = self.map_source_class_id(
                "interior.{}".format(annotation['category_id']))
            print('test')
            if class_id:
                mask_path = os.path.join(self.dataset_dir, self.image_info[image_id]["subfolder"], 
                                   'label0/data',
                                   str(self.image_info[image_id]["image_sub_id"])+'_nyu.png')
                m = imageio.imread(mask_path)
                m = self.get_mask(m, annotation["id"])
                #print(annotation["id"])
                #print(self.image_info[image_id]["image_sub_id"])
    ...
    def get_mask(self, mask_image, ann_id):
        """gets binary mask from instance map"""
        print(mask_image)
        binary_mask = np.where(mask_image == ann_id , 1, 0)
        return binary_mask
